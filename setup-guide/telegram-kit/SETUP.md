...
*The duct-tape-but-reliable Phase 4. Lets [PARTNER_NAME] text and voice-note their AI from their phone. Works TODAY, unlike the broken official plugin.*
...
- `poll-telegram.sh` — bash script that asks Telegram every minute "any new messages?" and writes each one to their AI's inbox folder as a markdown file. Voice notes get downloaded as `.oga` audio files.
- `com.partner.ai-name.telegram-poller.plist` — launchd job that runs the script every 60 seconds in the background.
- `check-telegram-skill.md` — a skill their AI uses to process the inbox when they ask "check my Telegram."

**How it behaves in practice:**

- [PARTNER_NAME] texts their bot at 14:02. At 14:02:something the poller grabs it and drops it into `~/[ai-name]/inbox/telegram/`.
- Next time they open Claude Code and say *"[AI_NAME], check my Telegram"*, their AI reads everything in the inbox, responds, marks messages processed.
...
### 1. Copy the script into their AI's folder
...
**Why:** anyone who finds the bot's username on Telegram can DM it. Without an allowlist, those messages would land in the AI's inbox. Allowlist = only [PARTNER_NAME]'s user ID accepted.

**Find their Telegram user ID:**

1. [PARTNER_NAME] DMs their bot once from their phone (*"hello"*).
2. You run (on their Mac, after saving the .env above):
   ```bash
   ...
   ```
3. In the output, find `"from": {"id": 123456789, ...}` — that number is their user ID.
...
- `[PARTNER_USERNAME]` — their macOS username (run `whoami` in Terminal if unsure).
- `[AI_NAME]` — the AI's folder name (e.g. `mira`).
- `[PARTNER]` — short identifier (e.g. `alex`).
...
[PARTNER_NAME] sends a test message from their phone. Within ~60 seconds you should see a `WROTE: ...` line in the log, and a new file in `~/[ai-name]/inbox/telegram/`.

### 6. Install the inbox-check skill

Copy `check-telegram-skill.md` to their AI's skills folder:
...
Now they can say *"[AI_NAME], check my Telegram"* and their AI processes the inbox.

---

## How to reply back on Telegram

When the AI processes an inbox message and wants to reply, it calls Telegram's `sendMessage` API directly. That's a one-line curl the AI can run itself:
...
(This is documented in the `check-telegram-skill.md` — their AI reads it and knows the pattern.)
...
- Is the token valid? `curl -s "https://api.telegram.org/bot${TOKEN}/getMe"` should return the bot info.

### Messages arrive but [PARTNER_NAME] doesn't see them

- Check the inbox: `ls ~/[ai-name]/inbox/telegram/`
- If files exist but their AI isn't reading them, the skill isn't installed — do step 6.
...
### Multiple AIs on one Mac conflicting
...
3. The AI's behaviour changes from "reads inbox on request" to "reacts in real time."
4. [PARTNER_NAME] doesn't have to re-learn anything on their end — bot username stays the same.
